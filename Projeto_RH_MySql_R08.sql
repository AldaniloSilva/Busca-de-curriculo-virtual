create database if not exists projeto_rh;
use projeto_rh;

# drop database projeto_rh;

CREATE TABLE if NOT EXISTS pais (
	id int(4) NOT NULL auto_increment,
    nome varchar(60) default null,
    sigla varchar(60) default null,
    primary key (id)    
...
CREATE TABLE IF NOT EXISTS estado (
    ...
    primary key (id),
    KEY fk_Estado_pais (pais)) ENGINE=InnoDB  DEFAULT CHARSET=latin1;
...
select * from cidade where nome = 'Mauá';

Create table if not exists Candidato (
id int(3) not null auto_increment,
nome varchar (60) default null,
nascimento int(4) default null,
cidade int (11) default null,
cargo varchar(170) default null,
telefone varchar(25) default null,
escolaridade varchar(60) default null,
email varchar(100) default null,
nome_arquivo varchar(80) default null,
primary key (id),
key fk_Candidato_cidade(cidade)) ENGINE=InnoDB, DEFAULT CHARSET=utf8;
...
INSERT INTO usuario (nome, senha, tipoAcesso) VALUES ('admin',aes_encrypt('admin','rh'),'Administrador');  


#select * from usuario where CAST(AES_DECRYPT(senha,'rh') AS char(20))='admin' ; 




/*
#ESSE SELECT É PARA VER A SENHA SEM CRIPTOGRAFIA
select id,nome,CAST(AES_DECRYPT(senha,'rh') AS char(20)) as senha,tipoAcesso 
from usuario

select id,nome,CAST(AES_DECRYPT(senha,'rh') AS char(20)) as senha,tipoAcesso 
from usuario where CAST(AES_DECRYPT(senha,'r') AS char(20))='admin' ; 

UPDATE usuario 
SET senha=aes_encrypt('654321','rh') WHERE id=3;

delete from usuario where id>0;

UPDATE candidato 
SET nome_arquivo="teste1.txt" WHERE id=1; 

UPDATE candidato 
SET nome_arquivo="teste2.txt" WHERE id=2;  

select * from estado;

select candidato.* from candidato inner join cidade on candidato.cidade=cidade.id 
inner join estado on cidade.estado=estado.id where estado.nome like '%Espir%';

delete from candidato where id>0;

select * from candidato;

#select * from cidade; 

select * from usuario where nome='bruno';

select candidato.nome, year(curdate()) - year(nascimento) as idade, cargo, cidade.nome from candidato inner join 
	cidade on candidato.cidade = cidade.id;

SELECT CAN.nome,CID.NOME FROM CANDIDATO CAN inner join CIDADE CID on can.cidade=cid.id WHERE CID.NOME='Diadema';

select * from cidade where nome = 'sao bernardo';

select max(id)+1 as proximo from candidato;

select * from candidato where cargo = 'ciclista'
or cidade = 5038 
and nascimento between 1970 and 1980;


select cidade.id, cidade.nome, estado.uf from cidade 
inner join estado on cidade.estado=estado.id 
where cidade.id = 8;

select * from cidade where nome ='diadema';

select * from candidato where cidade = '4859'

select c.id,c.nome,e.uf from cidade as c inner join estado as e 
on c.estado=e.id where c.id = 8;

select * from cidade where nome like 'sao%';

select * from usuario

select *  from candidato 
inner join cidade on candidato.cidade=cidade.id
where cidade.nome like '%diade%' 
and escolaridade like '%medio%' 
order by escolaridade;

select nome_arquivo from candidato where Email='emanuellada-mata@hotmail.com'
